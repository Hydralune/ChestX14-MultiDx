"""
ConvNeXt Large 模型 - 14头分类器
使用SOTA级别的架构组件：Spatial Attention、Label Correlation Module、Decoupled Heads
"""
...
class MultiLabelClassifier(nn.Module):
    """
    多标签分类模型 - ConvNeXt Large版本
    ConvNeXt Large -> Spatial Attention -> Global Pool -> Label Correlation -> 14 Separate Heads
    
    【ConvNeXt Large优势】
    - 更大的模型容量（约200M参数）
    - 现代化的架构设计（ConvNeXt是2022年SOTA CNN架构）
    - 在ImageNet上表现优异，迁移到医学图像效果好
    - 适合高精度训练场景
    """
    
    def __init__(self, model_name="convnext_large", num_classes=14, pretrained=True):
        """
        Args:
            model_name: 模型名称 ("convnext_large")
            num_classes: 类别数量
        ...
        # === 1. 加载 ConvNeXt Large Backbone ===
        print(f"正在加载 ConvNeXt Large (预训练: {pretrained})...")
        ...
        # ConvNeXt Large的特征维度
        backbone_dim = self.backbone.num_features  # 1536 for convnext_large
        
        print(f"ConvNeXt Large 特征维度: {backbone_dim}")
...
def create_model(model_name="convnext_large", num_classes=14, pretrained=True):
    """
    创建模型的便捷函数
    使用ConvNeXt Large作为backbone，适合高精度训练场景
    """
...
if __name__ == "__main__":
    # 测试代码
    print("正在测试 ConvNeXt Large 模型架构...")
    
    # 1. 测试模型实例化
    try:
        model = create_model("convnext_large", num_classes=14, pretrained=False)
        print("模型创建成功: ConvNeXt Large + Attention + Label Correlation + Decoupled Heads")
